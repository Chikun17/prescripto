        ...
        try {
            if (state === 'Admin') {
                ...
                if (data.success) {
                    ...
                    toast.error(data.message); // ✅ handles wrong password
                }
            }
        } catch (error) {
            toast.error(error.response?.data?.message || error.message); // ✅ handles server errors
        }
    }

  return (
    <form onSubmit={onSubmitHandller} className='min-h-[80vh] flex items-center'>
        <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[380px] sm:min-w-96 border round-xl text-[#5E5E5E] text-sm shadow-lg' >
